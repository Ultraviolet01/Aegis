...
const monitorPath = resolve(ROOT, 'agent/src/monitor.ts');
const executorPath = resolve(ROOT, 'agent/src/executor.ts');
const { Monitor } = await import(pathToFileURL(monitorPath).href);
...
async function main() {
  ...
  console.log('\nStep 5: Initializing Agent Monitor & Executor (DRY_RUN = false)...');
  const chainConfig = {
    id: 1952,
  ...
  // Wire OKX DEX quote client mock for live drawdown test
  ...
  if (pos2State.amount === expectedVal && claimsCount >= 1n) {
    ...
    console.log('  - Mock price drop ($2,000 -> $1,600) detected live by Agent   ');
